    ...
    public class HaarDetector
    {
        public struct DResults
        {
            public DResults(int SearchedSubRegionCount, int NOfObjects, Rectangle[] DetectedOLocs)
            {
                ...
                // Number of objects
                this.DetectedOLocs = DetectedOLocs;
        ...
        {
            HCascade = new HearCascade(OpenCVXmlStorage);
        }
        ...
        {
            HCascade = new HearCascade(XmlDoc);
        }
        ...
        private void CalculateCumSums8bpp(ref int[,] CumSum, ref long[,] CumSum2, ref BitmapData BitmapData, ref int Width, ref int Height)
        {
            ...
            int ByteCount = AbsOfStride * Height;
            ...
            while (i < ByteCount)
            {
                ...
                int CurRow = Convert.ToInt32(Math.Floor(Convert.ToDouble(k / Width)));
                int CurCol = k % Width;
                if (CurCol == 0)
                {
                    CurRowSum = 0;
        ...
        {
            int AbsOfStride = FastAbs(BitmapData.Stride);
            int ExtraBPerLine = AbsOfStride - Width * 3;
            int ScanWidthWP = AbsOfStride - ExtraBPerLine;
            // Scan width without padding
            IntPtr BmpDataPtr = BitmapData.Scan0;
            ...
            while (i < ByteCount)
            {
                ...
                GrayVal = GrayVal + 0.587f * Colors[i];
                // Green
                i = i + 1;

                GrayVal = GrayVal + 0.299f * Colors[i];
                // Red
                i = i + 1;
        ...
        public DResults Detect(ref Bitmap Bitmap, DetectionParams Parameters)
        {
            ...
            while (Scaler < Parameters.MaxScale)
            {
                ...
                float InvArea = Convert.ToSingle(1 / (WinWidth * WinHeight));
                // Inverse of the area

                int StepSize = Convert.ToInt32(WinWidth * Parameters.SlidingRatio);
                // Current step size
                for (var i = 0; i <= Width - WinWidth - 1; i += StepSize)
                {
                    for (var j = 0; j <= Height - WinHeight - 1; j += StepSize)
                    {
                        ...
                        bool Passed = true;
                        foreach (HearCascade.Stage Stage in HCascade.Stages)
                        {
                            float StageVal = 0;
                            foreach (HearCascade.Tree Tree in Stage.Trees)
                            {
                                HearCascade.Node CurNode = Tree.Nodes[0];
                                while (true)
                                {
                                    int RectSum = 0;
                                    foreach (HearCascade.FeatureRect FeatureRect in CurNode.FeatureRects)
                                    {
        ...
        private bool AreTheyNested(ref Rectangle Rectangle1, ref Rectangle Rectangle2, ref float SizeMultForNesRectCon)
        {
            ...
            int MaxHorDist = Convert.ToInt32(SizeMultForNesRectCon * Rectangle1.Width);
            int MaxVertDist = Convert.ToInt32(SizeMultForNesRectCon * Rectangle1.Height);
